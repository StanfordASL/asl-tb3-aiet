...
class ICPNode(Node):
    ...
    def pcd_callback(self, msg: PointCloud2):
        ...
        pts = get_pcd_array_from_point_cloud(msg)
        transformed_pts = (self.lidar_pose[:3, :3] @ pts.T + self.lidar_pose[:3, 3:]).T
        ...
        if self.use_open3d:
            ...
            self.transformation = icp(np.asarray(pcd.points), np.asarray(self.prev_pcd.points), init_pose=self.transformation)
