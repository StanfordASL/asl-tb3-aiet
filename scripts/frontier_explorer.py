...
class FrontierExplorer(Node):
    ...
    def __init__(self) -> None:
        ...
        self.image_time = None # time of last image

    def explore(self, _) -> None:
        if not self.exploring or self.finished_exploring or self.state is None or self.occupancy is None:
            return
    ...
    def stop_sign_callback(self, msg: Bool):
        ...
        if self.image_time is not None:
            ...
            if elapsed_time >= self.stop_time and not self.exploring:
                self.get_logger().info("Resuming exploration!")
                self.exploring = True
                self.explore(None)
            elif elapsed_time >= self.stop_time + self.buffer_time:
